...
public class SupporterHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		sessionMappingManager.connected(Util.getHttpSessionId(session), session);
	}
		...
		try {
			...
			if (StringUtils.isNotEmpty(content)) {
				...
				if (StringUtils.isNotEmpty(sessionId)) {
					if (tb.getType().equals(MessageFactory.MESSAGE_TYPE_IMAGE_TALK_TO_CUSTOMER)) {
						ImageMeta im = Util.parseImgString(tb.getContent().get(Message.CONTENT_KEY_TALK_CONTENT));
						String path = Util.imageUploadPath(im.getSuffix());
						Util.GenerateImage(im.getContent(), path);
						tb.getContent().put(Message.CONTENT_KEY_TALK_CONTENT, Constant.IMAGE_ACCESS_PATH + path.substring(path.lastIndexOf("/") + 1));
					...
					sendMessage = messageFactory.generateMessage(talker, session, type, tb.getContent().get(Message.CONTENT_KEY_TALK_CONTENT));
				}
			}
			...
			if (talker != null && talker.isOpen()) {
				...
				//发送聊天信息
				talkService.saveTalk(session, talker, domainType, tb.getContent().get(Message.CONTENT_KEY_TALK_CONTENT));
