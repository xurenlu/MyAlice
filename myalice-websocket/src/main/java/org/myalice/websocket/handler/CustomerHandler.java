	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {		
		...
		try {
			...
			if (talker == null || !talker.isOpen()) {
				...
				if (tb != null) {
					if (tb.getType().equals(MessageFactory.MESSAGE_TYPE_IMAGE_TALK_TO_SUPPORTER)) {
					...
					talkService.saveTalk(session, null, talkDomainType, tb.getContent().get(Message.CONTENT_KEY_TALK_CONTENT));
			...
			//发送聊天内容
				...
				talkService.saveTalk(session, talker, talkDomainType, tb.getContent().get(Message.CONTENT_KEY_TALK_CONTENT));
